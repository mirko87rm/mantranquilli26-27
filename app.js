...
function toLocalInputValue(iso){if(!iso)return '';const d=new Date(iso);if(Number.isNaN(d.getTime()))return '';const pad=n=>String(n).padStart(2,'0');return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`}
function fromLocalInputValue(v){if(!v)return null;const d=new Date(v);return Number.isNaN(d.getTime())?null:d.toISOString()}
function formatSchedule(iso){if(!iso)return 'Non programmato';return new Date(iso).toLocaleString('it-IT',{dateStyle:'short',timeStyle:'short'})}
function calculateMarketOpen(){
  if(!marketSession)return false;
  const open=marketSession.open_at?new Date(marketSession.open_at).getTime():null;
  const close=marketSession.close_at?new Date(marketSession.close_at).getTime():null;
  const now=Date.now();
  if(open!==null&&close!==null)return now>=open&&now<close;
  return !!marketSession.is_open;
}
async function loadMarket(){
  const r=await db.from('market_sessions').select('*').order('created_at',{ascending:false}).limit(1);
  if(!r.error&&r.data?.length){
    marketSession=r.data[0];
    marketOpen=calculateMarketOpen();
    if($('marketOpenAt'))$('marketOpenAt').value=toLocalInputValue(marketSession.open_at);
    if($('marketCloseAt'))$('marketCloseAt').value=toLocalInputValue(marketSession.close_at);
  }
  renderMarket();
}
function renderMarket(){
  marketOpen=calculateMarketOpen();
  $('marketStatus').textContent=marketOpen?'APERTO':'CHIUSO';
  if($('homeMarket'))$('homeMarket').textContent=marketOpen?'APERTO':'CHIUSO';
  const schedule=marketSession?.open_at&&marketSession?.close_at?`Aperto dal ${formatSchedule(marketSession.open_at)} al ${formatSchedule(marketSession.close_at)}.`:'Nessuna programmazione impostata.';
  if($('marketSchedule'))$('marketSchedule').textContent=schedule;
  if($('adminMarketStatus'))$('adminMarketStatus').textContent=marketOpen?'APERTO':'CHIUSO';
  if($('adminMarketLabel'))$('adminMarketLabel').textContent=marketOpen?'MERCATO APERTO':'MERCATO CHIUSO';
  if($('adminMarketSchedule'))$('adminMarketSchedule').textContent=schedule;
}
$('saveMarketSchedule').onclick=async()=>{
  if(!currentProfile?.is_admin)return;
  const open=fromLocalInputValue($('marketOpenAt').value);
  const close=fromLocalInputValue($('marketCloseAt').value);
  if(!open||!close)return show('Inserisci sia la data/ora di apertura sia quella di chiusura.',true);
  if(new Date(open)>=new Date(close))return show('La chiusura deve essere successiva all’apertura.',true);
  const payload={name:marketSession?.name||'Mercato 1',is_open:false,max_players_per_team:5,open_at:open,close_at:close};
  let r;
  if(marketSession?.id)r=await db.from('market_sessions').update(payload).eq('id',marketSession.id).select('*').single();
  else r=await db.from('market_sessions').insert(payload).select('*').single();
  if(r.error){$('adminFeedback').textContent=r.error.message;$('adminFeedback').classList.add('error');return}
  marketSession=r.data;marketOpen=calculateMarketOpen();renderMarket();
  $('adminFeedback').textContent='Programmazione salvata. Il mercato si aprirà e chiuderà automaticamente agli orari impostati.';
  $('adminFeedback').classList.remove('error');
  await loadAdminPanel();
};
setInterval(()=>{if(marketSession){const before=marketOpen;marketOpen=calculateMarketOpen();if(before!==marketOpen)renderMarket();}},30000);
...
async function loadAdminPanel(){
  ...
  const schedule=marketSession?.open_at&&marketSession?.close_at?`Apertura: ${formatSchedule(marketSession.open_at)} · Chiusura: ${formatSchedule(marketSession.close_at)}`:'Nessuna programmazione impostata.';
  $('adminMarketSchedule').textContent=schedule;
  if($('marketOpenAt')&&!$('marketOpenAt').value)$('marketOpenAt').value=toLocalInputValue(marketSession?.open_at);
  if($('marketCloseAt')&&!$('marketCloseAt').value)$('marketCloseAt').value=toLocalInputValue(marketSession?.close_at);
}
